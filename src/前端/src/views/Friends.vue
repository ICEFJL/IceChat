...
import {addFriendService, flashFriendService, removeFriendService, searchFriendService} from "@/api/friend.js";
...
const flashFreind=async () => {
  if(router.currentRoute.value.fullPath!=='/friends'){
      clearInterval(timer)
    }
...
const addFriend=async (row) => {
  let result = await addFriendService(userInfo.userId,row.userId);
  if(result.code===0){
  ...
  <div class="flex-container">
    <el-container style="width: auto;height: 100%">
      <el-card class="page-container">
        ...
        <el-table class="groupList" :data="friends" style="width: 100%" @row-dblclick="toLeaveMessage">
